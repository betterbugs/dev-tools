...
const DecimalToAsciiConverter = () => {
  ...
  const convertDecimalToAscii = (decimalInput: string): string => {
    try {
      // Split by common delimiters and clean up
      const numbers = decimalInput
        .split(/[\s,;|]+/)
        .map(num => num.trim())
        .filter(num => num.length > 0)
        .map(num => parseInt(num, 10));

      // Validate all numbers are valid ASCII (0-255)
      const invalidNumbers = numbers.filter(num => isNaN(num) || num < 0 || num > 255);
      if (invalidNumbers.length > 0) {
        return `Error: Invalid decimal values found. ASCII values must be between 0-255.`;
      }
      ...
      return "Error: Invalid input format. Please enter decimal numbers separated by spaces, commas, semicolons, or pipes.";
    }
  };
  ...
  // No swap or alternate direction in this dedicated tool
          ...
          <div className="w-full bg-[#FFFFFF1A] rounded-2xl shadow-lg p-8">
            <div className="md:w-[900px] mx-auto">
              ...
              <div className="flex items-center gap-4">
                <h3 className="text-base font-medium">Decimal to ASCII</h3>
              </div>
                ...
                <div className="w-full">
                  ...
                  <div className="relative">
                    <textarea
                      ...
                      placeholder={
                        "Enter decimal numbers (0-255) separated by spaces, commas, semicolons, or pipes. Example: 72 101 108 108 111"
                      }
                      className={`${DevelopmentToolsStyles.scrollbar} w-full min-h-[120px] bg-black !border !border-[#222222] p-5 rounded-xl`}
                    />
                  </div>
                  <p className="text-xs text-white/60 mt-2">
                    Tip: Enter decimal values (0-255) separated by spaces, commas, semicolons, or pipes. Each number represents an ASCII character.
                  </p>
                </div>
                ...
                <div className="w-full">
                  ...
                  <div className="relative">
                    <textarea
                      readOnly
                      value={output}
                      placeholder={"ASCII characters will appear here..."}
...
export default DecimalToAsciiConverter;

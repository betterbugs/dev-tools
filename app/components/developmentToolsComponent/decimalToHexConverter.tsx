...
const DecimalToHexConverter: React.FC = () => {
  ...
  const onDownload = useCallback(() => {
    ...
    a.href = url; a.download = "decimal-to-hex.txt"; document.body.appendChild(a); a.click(); document.body.removeChild(a); URL.revokeObjectURL(url);
  }, [output]);
          ...
          <div className="md:w-[900px] mx-auto">
            ...
            <div className="grid md:grid-cols-2 grid-cols-1 gap-6">
              <div>
                ...
                <textarea value={input} onChange={(e) => setInput(e.target.value)} placeholder="e.g. 255" className="w-full h-44 bg-black rounded p-3 font-mono text-sm border border-white/20" />
                <div className={`text-xs mt-2 ${error ? "text-red-400" : "text-white/60"}`}>{hint}</div>
              </div>
              <div className="space-y-2">
                <div className="flex items-center justify-between mb-2">
                  <span className="text-sm text-white/80">Output (Hexadecimal)</span>
                </div>
            ...
            <div className="mt-6 bg-[#FFFFFF1A] rounded-2xl p-4">
              ...
              <label className="flex items-center gap-2 text-sm md:mt-2">

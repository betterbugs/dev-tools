...
const MilesToKmConverter: React.FC = () => {
  ...
  const convert = useCallback(() => {
    ...
    setError("");
    const km = n * MI_TO_KM;
    const out = precision ? km.toFixed(precision) : String(km);
    setOutput(out);
  }, [input, precision]);
  ...
  const onDownload = useCallback(() => {
    ...
    a.href = url; a.download = "miles-to-km.txt"; document.body.appendChild(a); a.click(); document.body.removeChild(a); URL.revokeObjectURL(url);
  }, [output]);
  ...
  const hint = useMemo(() => (error ? error : "1 mile = 1.609344 km"), [error]);
            ...
            <div className="grid md:grid-cols-2 grid-cols-1 gap-6">
              <div>
                <div className="flex items-center justify-between mb-2">
                  <span className="text-sm text-white/80">Input (Miles)</span>
                ...
                <textarea value={input} onChange={(e) => setInput(e.target.value)} placeholder="e.g. 1" className="w-full h-32 bg-black rounded p-3 font-mono text-sm border border-white/20" />
                <div className={`text-xs mt-2 ${error ? "text-red-400" : "text-white/60"}`}>{hint}</div>
              </div>
              <div className="space-y-2">
                <div className="flex items-center justify-between mb-2">
                  <span className="text-sm text-white/80">Output (Kilometers)</span>
                </div>

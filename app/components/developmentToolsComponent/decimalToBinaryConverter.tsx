...
const DecimalToBinaryConverter: React.FC = () => {
  const [input, setInput] = useState<string>("210");
  const [output, setOutput] = useState<string>("");
  const [autoUpdate, setAutoUpdate] = useState<boolean>(true);
  const [error, setError] = useState<string>("");
  const [groupSize, setGroupSize] = useState<4 | 8>(4);
  const [padTo, setPadTo] = useState<0 | 8 | 16 | 32>(8);

  const fileRef = useRef<HTMLInputElement>(null);

  const convert = useCallback(() => {
    const n = parseDecimal(input);
    if (n === null) { setError("Invalid decimal number"); setOutput(""); return; }
    setError("");
    const abs = Math.abs(n) >>> 0;
    const bin = abs.toString(2).padStart(padTo || 0, "0");
    const grouped = bin.replace(new RegExp(`(.{${groupSize}})`, "g"), "$1 ").trim();
    setOutput(n < 0 ? `-${grouped}` : grouped);
  }, [input, groupSize, padTo]);
  ...
  const onDownload = useCallback(() => {
    ...
    a.href = url; a.download = "decimal-to-binary.txt"; document.body.appendChild(a); a.click(); document.body.removeChild(a); URL.revokeObjectURL(url);
  }, [output]);
  ...
  const hint = useMemo(() => (error ? error : "Enter an integer. Negative supported."), [error]);
          ...
          <div className="md:w-[900px] mx-auto">
            ...
            <div className="grid md:grid-cols-2 grid-cols-1 gap-6">
              <div>
                <div className="flex items-center justify-between mb-2">
                  <span className="text-sm text-white/80">Input (Decimal)</span>
                ...
                <textarea value={input} onChange={(e) => setInput(e.target.value)} placeholder="e.g. 210" className="w-full h-44 bg-black rounded p-3 font-mono text-sm border border-white/20" />
            ...
            <div className="mt-6 bg-[#FFFFFF1A] rounded-2xl p-4">
              ...
              <div className="flex flex-wrap md:flex-nowrap items-center gap-6 md:mt-2">
                ...
                <div className="flex items-center gap-2 text-sm">
                  <span>Pad to:</span>
                  <select value={padTo} onChange={(e) => setPadTo(Number(e.target.value) as 0 | 8 | 16 | 32)} className="bg-black border border-white/20 rounded px-2 py-1">

...
const KmToMilesConverter: React.FC = () => {
  const [input, setInput] = useState<string>("1.609344");
  const [output, setOutput] = useState<string>("");
  const [autoUpdate, setAutoUpdate] = useState<boolean>(true);
  const [error, setError] = useState<string>("");
  const [precision, setPrecision] = useState<0 | 2 | 3 | 4>(2);

  const fileRef = useRef<HTMLInputElement>(null);

  const convert = useCallback(() => {
    const n = parseNumber(input);
    if (n === null) { setError("Invalid number"); setOutput(""); return; }
    setError("");
    const mi = n * KM_TO_MI;
    const out = precision ? mi.toFixed(precision) : String(mi);
    setOutput(out);
  }, [input, precision]);
  ...
  const onDownload = useCallback(() => {
    ...
    a.href = url; a.download = "km-to-miles.txt"; document.body.appendChild(a); a.click(); document.body.removeChild(a); URL.revokeObjectURL(url);
  }, [output]);
  ...
  const hint = useMemo(() => (error ? error : "1 km = 0.6213711922 miles"), [error]);
            ...
            <div className="grid md:grid-cols-2 grid-cols-1 gap-6">
              <div>
                <div className="flex items-center justify-between mb-2">
                  <span className="text-sm text-white/80">Input (Kilometers)</span>
                ...
                <textarea value={input} onChange={(e) => setInput(e.target.value)} placeholder="e.g. 1.609344" className="w-full h-32 bg-black rounded p-3 font-mono text-sm border border-white/20" />
                <div className={`text-xs mt-2 ${error ? "text-red-400" : "text-white/60"}`}>{hint}</div>
              </div>
              <div className="space-y-2">
                <div className="flex items-center justify-between mb-2">
                  <span className="text-sm text-white/80">Output (Miles)</span>
                </div>
                <textarea value={output} readOnly className="w-full h-32 bg-black rounded p-3 font-mono text-sm border border-white/20" />
              </div>
            </div>

            <div className="mt-6 bg-[#FFFFFF1A] rounded-2xl p-4">
              <h3 className="font-semibold mb-3">Options</h3>
              <div className="flex items-center gap-3 text-sm md:mt-2">
                <span>Precision:</span>
                <select value={precision} onChange={(e) => setPrecision(Number(e.target.value) as 0 | 2 | 3 | 4)} className="bg-black border border-white/20 rounded px-2 py-1">
